...
/* Copyright 2012-2013 James F. Bellinger <http://www.zer7.com/software/hidsharp>
   ...
   Modified for HidSharper - Windows-only support. */
#endregion
...
namespace XOutputRedux.HidSharper.Platform
{
    sealed class HidSelector
    {
        ...
        static HidSelector()
        {
            var hidManager = new Windows.WinHidManager();
            if (!hidManager.IsSupported)
            {
                throw new PlatformNotSupportedException("HidSharper only supports Windows.");
            }
