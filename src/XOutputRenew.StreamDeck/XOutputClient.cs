...
/// <summary>
/// Client for communicating with XOutputRenew via named pipe IPC.
...
public static class XOutputClient
{
    ...
    private const string ExeName = "XOutputRenew.exe";
    ...
    /// <summary>
    /// Status returned from XOutputRenew.
    /// </summary>
    ...
    /// <summary>
    /// Checks if XOutputRenew GUI is running (IPC server available).
    /// </summary>
    ...
    /// <summary>
    /// Gets the current status from the running XOutputRenew instance.
    /// </summary>
    ...
    private static async Task EnsureGuiRunningAsync()
    {
        ...
        Logger.Instance.LogMessage(TracingLevel.INFO, "XOutputRenew not running, starting minimized...");
        ...
        try
        {
            ...
            while ((DateTime.Now - startTime).TotalMilliseconds < StartupWaitMs)
            {
                ...
                if (IsGuiRunning())
                {
                    Logger.Instance.LogMessage(TracingLevel.INFO, "XOutputRenew started successfully");
                    return;
                }
            }

            Logger.Instance.LogMessage(TracingLevel.WARN, "XOutputRenew started but IPC not responding");
        }
        catch (Exception ex)
        {
            Logger.Instance.LogMessage(TracingLevel.ERROR, $"Failed to start XOutputRenew: {ex.Message}");
            throw;
        }
    }
