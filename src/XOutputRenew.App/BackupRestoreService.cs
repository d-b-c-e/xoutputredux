...
public static class BackupRestoreService
{
    ...
    {
        return $"XOutputRenew-Backup-{DateTime.Now:yyyy-MM-dd-HHmmss}.xorbackup";
    }
    ...
    {
        var tempDir = Path.Combine(Path.GetTempPath(), $"XOutputRenew-Backup-{Guid.NewGuid():N}");
    ...
    {
        var tempDir = Path.Combine(Path.GetTempPath(), $"XOutputRenew-Restore-{Guid.NewGuid():N}");
