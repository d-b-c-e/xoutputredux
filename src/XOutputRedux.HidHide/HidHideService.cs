...
public class HidHideDevice
{
    ...
    /// <summary>
    /// Reduces a device path to a stable HARDWARE identity — the part that survives
    /// re-enumeration — for keep-list matching when the exact instance path no longer
    /// resolves.
    ///
    /// Instance paths are not stable. The trailing segment
    /// (<c>d&amp;20e2af56&amp;0&amp;0000</c>) changes with the USB port and enumeration
    /// order, and an XInput pad's <c>IG_xx</c> infix tracks its XInput slot, which moves
    /// whenever the device is replugged or its controller mode is cycled — an X-Arcade
    /// panel was observed shifting from <c>IG_00/01/02</c> to <c>IG_04/05</c> within a
    /// single session. A profile saved before the move matches nothing afterwards.
    ///
    ...
    /// What is dropped: the instance segment, <c>IG_</c>, <c>REV_</c>, and the interface
    /// class GUID.
    ///
    /// Accepts all three forms a profile may hold — <c>HID\...</c>, <c>USB\...</c>, and
    /// the <c>\\?\hid#...#{guid}</c> symbolic link. Returns null when the path carries no
    ...
    /// its <see cref="MatchPaths"/>. A composite device yields both the interface-level
    /// identity (<c>VID_346E&amp;PID_0024&amp;MI_02</c>, from its HID path) and the
    /// container-level one (<c>VID_346E&amp;PID_0024</c>), so a profile that saved either
    /// still recognises it.
    /// </summary>
    public static IEnumerable<string> IdentityKeys(HidHideDevice d) =>
        MatchPaths(d)
